...
configure_firewall() {
  log "Versuche Port ${PORT} in der Firewall freizugeben"

  if command -v ufw >/dev/null 2>&1; then
    as_root ufw allow "${PORT}/tcp" || warn "ufw-Regel konnte nicht gesetzt werden"
    return
  fi

  if command -v firewall-cmd >/dev/null 2>&1; then
    as_root firewall-cmd --permanent --add-port="${PORT}/tcp" || warn "firewalld permanent rule fehlgeschlagen"
    as_root firewall-cmd --reload || warn "firewalld reload fehlgeschlagen"
    return
  fi

  if command -v iptables >/dev/null 2>&1; then
    as_root iptables -C INPUT -p tcp --dport "${PORT}" -j ACCEPT >/dev/null 2>&1 || \
      as_root iptables -I INPUT -p tcp --dport "${PORT}" -j ACCEPT || warn "iptables-Regel konnte nicht gesetzt werden"
    return
  fi

  warn "Keine unterstützte lokale Firewall erkannt; Portfreigabe ggf. manuell durchführen"
}
...
print_access_urls() {
  ...
  if [ -n "${ips}" ]; then
    for ip in ${ips}; do
      log "Erreichbar unter: http://${ip}:${PORT}"
    done
  fi

  log "Falls ein Cloud-Provider genutzt wird, muss Port ${PORT}/tcp zusätzlich in der Provider-Firewall/Security-Group freigegeben sein."
}
